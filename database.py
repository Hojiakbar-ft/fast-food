...
def create_product_table():
    ...
    CREATE TABLE IF NOT EXISTS products(
     ...
     product_description Varchar(150) NOT NULL UNIQUE,
...
# category_info = ['Бургеры', 'Шаурма', 'Хот-доги', 'Напитки', 'Салаты', 'Закуски']
#
# for i in category_info:
#     if insert_to_categories(i):
#         print(f'{i} - successfully inserted!')

def insert_to_products(category_id, product_name, product_price, product_description, product_image):
    database = sqlite3.connect('shop.db')
    cursor = database.cursor()
    cursor.execute('''
    INSERT INTO products(category_id, product_name, product_price, product_description, product_image)
    VALUES (?, ?, ?, ?, ?)
    ''', (category_id, product_name, product_price, product_description, product_image))
    database.commit()
    database.close()
    return True

# product_info = [
#     [1, 'Чизбургер', 30000, 'Обычный гамбургер с сыром вкусно как всегда', 'media/Burgers/img.png'],
#     [2, 'Шаурма куринная острая', 32000, 'Обычный лаваш с перцом (Сразу в туалет)', 'media/shaurmas/img.png'],
#     [3, 'Дабл хот-дог', 19000, 'Хот дог с двумя сосисками', 'media/xotdogs/img.png'],
#     [4, 'Coca-cola', 9000, '0.5L Кола', 'media/drinks/img.png'],
#     [5, 'Цезарь', 21000, 'Салат с курицей и помидорами (Вкусно очень)', 'media/Salads/img.png'],
#     [6, 'Картошка фри', 14999, 'Small вкусно и вкусно', 'media/snacks/img.png']
# ]
#
#
# for i in product_info:
#     if insert_to_products(i[0], i[1], i[2], i[3], i[4]):
#         print(f'{i[1]} - successfully inserted!!!')
#
...
def create_table_orders():
    ...
    CREATE TABLE IF NOT EXISTS orders(
        ...
        order_check_id INTEGER REFERENCES order_checks(order_check_id),
...
def get_order_check_ids(cart_id):
    ...
    WHERE cart_id = ?
    ''', (cart_id,))
    order_check_ids = cursor.fetchall()
    database.close()
    return order_check_ids
