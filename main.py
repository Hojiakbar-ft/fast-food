...
async def command_start(message: Message):
    ...
    await message.answer(f'👋 Саламу алейкум {full_name}! \nДобро пожаловать в наш 🍟Kamil&food🌭')
...
async def show_details_product(call: CallbackQuery):
    chat_id = call.message.chat.id
    msg_id = call.message.message_id
...
async def create_order(call: CallbackQuery):
    ...
    date_order = datetime.datetime.now().strftime('%d/%m')
...
async def show_order_history(message: Message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)
    order_check_ids = get_order_check_ids(cart_id)
    ...
    for idx, (order_check_id,) in enumerate(order_check_ids, start=1):
        order_check = get_order_check(cart_id, order_check_id)
        text += f"{idx}. Заказ №{order_check_id}\n"
        text += f"Время: {order_check[4]}\nДата: {order_check[5]}\n"
        text += f"Товаров: {order_check[3]}\n💰 Общая сумма: {order_check[2]} сум\n"
        text += f"/order_{order_check_id}\n\n"

    await message.answer(text)

@dp.message(lambda message: message.text.startswith("/order_"))
async def show_order_details(message: Message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)

    try:
        order_check_id = int(message.text.split("_")[1])
    except (IndexError, ValueError):
        await message.answer("Неверный формат запроса.")
        return

    order_check_ids = [i[0] for i in get_order_check_ids(cart_id)]
    if order_check_id not in order_check_ids:
        await message.answer("Заказ не найден.")
        return

    order_check = get_order_check(cart_id, order_check_id)
    detail_order_info = get_detail_order(order_check_id)

    text = f"Детали заказа №{order_check_id}\n"
    text += f"Время: {order_check[4]}\nДата: {order_check[5]}\n\n"

    for idx, (product_name, quantity, final_price) in enumerate(detail_order_info, start=1):
        text += f"{idx}. {product_name}\nКоличество: {quantity}\nЦена: {final_price} сум\n\n"

    text += f"Всего товаров: {order_check[3]}\n💰 Итого: {order_check[2]} сум"
